...
RedLeftAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Path path = new Path().addPoint(new PathPoint(-20, 23)).headingMethod(Path.HeadingMethod.CONSTANT_ANGLE);
        ...
        Path path2 = new Path().addPoint(new PathPoint(-30, 25)).addPoint(new PathPoint(-64, 14)).headingMethod(Path.HeadingMethod.CONSTANT_ANGLE).positionPrecision(3).headingPrecision(200).constantHeading(90);
        ...
        while (timer.milliseconds() - duckTimer < 1200) {
            float speed = (float) (.0005*(timer.milliseconds() - duckTimer)+.1);
            robot.setDuckSpinnerPower(-speed);
        }
        while (timer.milliseconds() - duckTimer < 2000 && timer.milliseconds() - duckTimer >= 1100) {
            robot.setDuckSpinnerPower(-0.5F);
        }
        while (timer.milliseconds() - duckTimer < 2050 && timer.milliseconds() - duckTimer >= 2000) {
            robot.setDuckSpinnerPower((float)0.1);
        }
        robot.setDuckSpinnerPower(0);

        path2 = new Path().addPoint(new PathPoint(-67, 34)).headingMethod(Path.HeadingMethod.CONSTANT_ANGLE).positionPrecision(1).headingPrecision(10).constantHeading(90);
        AutoHelper.followCurvePath(path2, 0.5, 0.1, this, robot, telemetry, dashboard,  startingPose, currentPose);
        robot.initIntakeServo();
    ...
    public void leftAuto() {
        ...
        Path path = new Path().addPoint(new PathPoint(-20, 20)).headingMethod(Path.HeadingMethod.CONSTANT_ANGLE).positionPrecision(2).headingPrecision(15);
        AutoHelper.followCurvePath(path, 1, 0.1, this, robot, telemetry, dashboard,  startingPose, currentPose);
